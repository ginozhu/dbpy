	...
	def PageParser(self,page): 
		...
		if self.add==True: 
			for direccion in directions: 
				if not direccion.endswith(".js") and not direccion.endswith(".css") and not direccion.endswith(".exe") and not direccion.endswith(".pdf"): 
...
class douban(object): 
	...
	def signin(self): 
		...
		req = urllib2.Request('http://www.douban.com/login', urllib.urlencode(params)) 
	
		print 'requesting..'
		r = self.opener.open(req) 
		
		print 'Logged on successfully!' 
		print self.cj.save('douban.cookie') 
		self.signed_in = True 
