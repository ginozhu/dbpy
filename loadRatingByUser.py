...
def crawlOneUser(userId, startFrom):
    ...
    while True:
        try:
            collectPage = urllib2.urlopen("http://movie.douban.com/people/" + userId + "/collect?start=" + str(
                startFrom) + "&sort=time&rating=all&filter=all&mode=list-view", timeout=30).read().encode('utf-8')
        except:
            time.sleep(30)
            continue
        break
